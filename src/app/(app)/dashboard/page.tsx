...
export default async function DashboardPage({ searchParams }: { searchParams: { propertyId?: string } }) {
  ...
  const selectedPropertyId = searchParams.propertyId || allProperties[0]?.id || null;

  // Helper function to fetch all data for a specific property in parallel.
  const getDashboardData = async (propertyId: string) => {
    try {
      ...
      const allTenantsForAnalytics = await getTenants(); // This is needed for cross-property analytics components.
      ...
      return {
        ...
        maintenanceRequests,
        payments,
        allTenants: allTenantsForAnalytics,
      };
    } catch (error) {
                ...
                <CardContent className="space-y-6">
                    <Tabs defaultValue="status-analytics" className="w-full">
                        ...
                        <TabsContent value="occupancy-analytics">
                            {data.selectedProperty && <UnitAnalytics property={data.selectedProperty} tenants={data.allTenants} />}
                        </TabsContent>
                        <TabsContent value="orientation-analytics">
                            {data.selectedProperty && <OrientationAnalytics property={data.selectedProperty} tenants={data.allTenants} />}
                        </TabsContent>
                    </Tabs>
                </CardContent>
            </Card>
                ...
                <CardContent>
                    ...
                    {data.maintenanceRequests.filter(r => r.status !== 'Completed').slice(0, 3).map(req => (
                        ...
                        <div className="flex flex-col min-w-0 flex-1 mr-4">
                            <span className="font-medium truncate">{data.allTenants.find(t=>t.id === req.tenantId)?.name || 'Unknown'} - <span className="text-muted-foreground">{data.selectedProperty?.name}</span></span>
